    ...
    <section class="form-card">
      ...
      <form id="loginForm">
        ...
        <div style="position:relative">
          <input id="logPass" class="input" name="password" type="password" placeholder="Contraseña" required style="padding-right:40px">
          <button type="button" onclick="togglePassword('logPass', this)" style="position:absolute;right:10px;top:12px;background:none;border:none;cursor:pointer;font-size:16px">👁️</button>
        </div>
  ...
  layout(`
    <section class="form-card">
      ...
      <form id="registerForm">
        ...
        <input class="input" name="email" type="email" placeholder="Correo electrónico" required>
        <div style="position:relative">
          <input id="regPass" class="input" name="password" type="password" minlength="8" placeholder="Contraseña (mín 8 carac.)" required style="padding-right:40px">
          <button type="button" onclick="togglePassword('regPass', this)" style="position:absolute;right:10px;top:12px;background:none;border:none;cursor:pointer;font-size:16px">👁️</button>
        </div>
        <div style="position:relative">
          <input id="regConf" class="input" name="confirmPassword" type="password" minlength="8" placeholder="Repite la contraseña" required style="padding-right:40px">
          <button type="button" onclick="togglePassword('regConf', this)" style="position:absolute;right:10px;top:12px;background:none;border:none;cursor:pointer;font-size:16px">👁️</button>
        </div>
...
async function renderUser() {
  ...
  layout(`
    ...
    <section class="section">
      ...
      <div class="grid">${cfg.platforms.length ? cfg.platforms.map(p => `<a class="platform" href="${esc(p.link)}" target="_blank" rel="noopener"><div class="logo">${esc((p.name || 'P').slice(0, 1).toUpperCase())}</div><strong>${esc(p.name)}</strong><span>Ir a la plataforma ↗</span></a>`).join('') : '<div class="card empty">Las plataformas aparecerán aquí cuando las configures.</div>'}</div>
    </section>
    <section class="section card">
      ...
      <div class="grid">${cfg.platforms.map(p => `<button class="btn cyan redeem" data-id="${p.id}" ${me.coins < cfg.threshold ? 'disabled' : ''}>Solicitar PIN de ${esc(p.name)}</button>`).join('')}</div>
    </section>
    <section class="section card">
      <div class="section-head"><div><p class="eyebrow">MIS SOLICITUDES</p><h2>Historial</h2></div></div>
      ${red.redemptions.length ? red.redemptions.map(r => `<div class="offer"><div><h3>${esc(r.platform_name)}</h3><p>${esc(r.requested_at)} · ${esc(r.status)}</p></div><span class="pill">${fmt(r.coins_cost)} monedas</span></div>`).join('') : '<div class="empty">Aún no tienes solicitudes.</div>'}
    </section>
  `);
  ...
  layout(`
    <section class="form-card">
      ...
      <form id="setupForm">
        ...
        <div style="position:relative">
          <input id="setPass" class="input" name="password" type="password" minlength="8" placeholder="Crea una contraseña" required style="padding-right:40px">
          <button type="button" onclick="togglePassword('setPass', this)" style="position:absolute;right:10px;top:12px;background:none;border:none;cursor:pointer;font-size:16px">👁️</button>
        </div>
        <div style="position:relative">
          <input id="setConf" class="input" name="confirmPassword" type="password" minlength="8" placeholder="Repite la contraseña" required style="padding-right:40px">
          <button type="button" onclick="togglePassword('setConf', this)" style="position:absolute;right:10px;top:12px;background:none;border:none;cursor:pointer;font-size:16px">👁️</button>
        </div>
